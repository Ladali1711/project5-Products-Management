...
const creatingCart = async (req, res) => {
  try {
    ...
    if (!validator.isValidBody(requestBody)) {
      return res
        .status(400)
        .send({ status: false, msg: "Please provide valid request body" });
    }
    ...
    if (
      !(validator.isValidobjectId(productId) || validator.isValid(productId))
    ) {
      return res
        .status(400)
    ...
    if (!validator.validQuantity(quantity)) {
      return res
        ...
        .send({
          status: false,
          msg: "Please provide valid quantity & it must be greater than zero.",
        });
    }
    ...
    if (!findUser) {
      return res
        .status(400)
        .send({ status: false, msg: `User doesn't exist by ${userId}` });
    }
    ...
    if (!findProduct) {
      return res
        .status(400)
        .send({ status: false, msg: `Product doesn't exist by ${productId}` });
    }
...
const updateCart = async function (req, res) {
  try {
    ...
    if (!validator.isValidBody(data)) {
      return res
        .status(400)
        .send({ status: false, msg: "Enter value to be updating.." });
    }
    ...
    if (!validator.isValidobjectId(cartId)) {
      return res
        .status(400)
        .send({ status: false, msg: "cartId is not a valid objectId" });
    }
    ...
    if (!validator.isValidobjectId(productId)) {
      return res
        .status(400)
        .send({ status: false, msg: "productId is not a valid objectId" });
    }
    ...
    if (!userDetails) {
      return res
        .status(404)
        .send({ status: false, msg: "user not exist with this userId" });
    }
        ...
        if (cartDetails.items[i].productId == productId) {
          ...
          if (cartDetails.items[i].quantity > 1) {
            ...
            let updateCartDetails = await cartModel.findOneAndUpdate(
              { _id: cartId },
              { items: cartDetails.items, totalPrice: newPrice },
              { new: true }
            );
            return res
              .status(200)
      ...
      for (let i = 0; i < cartDetails.items.length; i++) {
        if (cartDetails.items[i].productId == productId) {
          let newPrice =
            ...
            productDetails.price * cartDetails.items[i].quantity;
...
const getCart = async (req, res) => {
  try {
    ...
    if (!validator.isValidobjectId(userId)) {
      return res
        .status(400)
        .send({ status: false, msg: "Invalid userId in params." });
    }
    const findUser = await userModel.findById({ _id: userId });
    if (!findUser) {
      return res
        .status(400)
        .send({ status: false, msg: `User doesn't exists by ${userId} ` });
    }
    ...
    if (!findCart) {
      return res
        .status(400)
        .send({ status: false, msg: `Cart doesn't exists by ${userId} ` });
    }
...
const deleteCart = async (req, res) => {
  try {
    ...
    if (!findUser) {
      return res
        .status(400)
        .send({ status: false, msg: `User does not exits by ${userId}` });
    }
    ...
    if (!findCart) {
      return res
        .status(400)
        .send({ status: false, msg: `Cart doesn't exits by ${userId}` });
    }
