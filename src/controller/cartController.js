...
const getCart = async (req, res) => {
  try {
    ...
    if (!findCart) {
      return res
        .status(400)
        .send({ status: false, message: `Cart doesn't exists by ${userId} ` });
    }
...
const deleteCart = async (req, res) => {
  try {
    ...
    if (!findUser) {
      return res
        .status(400)
        .send({ status: false, message: "User does not exits by ${userId}" });
    }
    ...
    if (!findCart) {
      return res
        .status(400)
        .send({ status: false, message: "Cart doesn't exits by ${userId}" });
    }
