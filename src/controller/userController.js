...
const createUser = async function (req, res) {
  try {
    ...
    if (!isValidRequestBody(userBody)) {
      return res
        .status(400)
        .send({ status: false, error: "user details are missing" });
    }
    ...
    if (!isValid(address)) {
      return res
        .status(400)
        .send({ status: false, msg: "please provide  valid address" });
    }
    if (Object.keys(address.shipping) == 0) {
      return res
        .status(400)
        .send({ status: false, msg: "please provide shipping address" });
    }
    if (!isValid(address.shipping.street)) {
      return res
        .status(400)
        .send({ status: false, msg: "please provide street" });
    }
    if (!isValid(address.shipping.city)) {
      return res
        .status(400)
        .send({ status: false, msg: "please provide city" });
    }
    if (!isValid(address.shipping.pincode)) {
      return res
        .status(400)
        .send({ status: false, msg: "please provide pincode" });
    }
    if (Object.keys(address.billing) == 0) {
      return res
        .status(400)
        .send({ status: false, msg: "please provide billing address" });
    }
...
const userLogin = async function (req, res) {
  try {
    ...
    return res
      .status(200)
      .send({ status: true, message: "User login successfull", token: token });
  } catch (err) {
    res.status(500).send({ status: false, msg: err.message });
  }
};
...
const getUserById = async function (req, res) {
  try {
    ...
    return res
      .status(200)
      .send({ status: true, message: "User profile details", data: userInfo });
  } catch (error) {
