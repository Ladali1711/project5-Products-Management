...
const createProduct = async function (req, res) {
  try {
    ...
    if (files && files.length > 0) {
      ...
      return res
        .status(201)
        .send({ status: true, msg: "Product updated", data: productData });
    } else {
      return res
        .status(400)
...
const getProduct = async function (req, res) {
  try {
    ...
    // To search size
    if (size) {
      let sizeSearch = await productModel
        .find({ availableSizes: size, isDeleted: false })
        .sort({ price: priceSort });

      if (sizeSearch.length !== 0) {
        return res
          .status(200)
          .send({ status: true, msg: "Success", data: sizeSearch });
      } else {
        return res
          .status(400)
          .send({ status: false, msg: "No products exist" });
      }
    }

    // To find products with name
    if (name) {
      let nameSearch = await productModel
        .find({ title: { $regex: name }, isDeleted: false })
        .sort({ price: priceSort });

      if (nameSearch.length !== 0) {
        return res
          .status(200)
          .send({ status: true, msg: "Success", data: nameSearch });
      } else {
        return res
          .status(400)
          .send({ status: false, msg: "No products exist" });
    ...
    if (priceLessThan || priceGreaterThan) {
      let searchPrice = await productModel
        .find({ price: data, isDeleted: false })
        .sort({ price: priceSort });

      if (searchPrice.length !== 0) {
        return res
          .status(200)
          .send({ status: true, msg: "Success", data: searchPrice });
      } else {
        return res
          .status(400)
          .send({ status: false, msg: "No products exist" });
    ...
    if (finalProduct !== 0) {
      return res
        .status(200)
        .send({ status: true, msg: "Success", data: finalProduct });
    } else {
...
const getProductById = async function (req, res) {
  try {
    ...
    return res
      .status(200)
      .send({ status: false, msg: "success", productDetails: productDetails });
  } catch (err) {
